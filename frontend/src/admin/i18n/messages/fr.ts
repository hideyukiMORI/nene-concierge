import type { MessageCatalog } from './en'

export const fr: Partial<MessageCatalog> = {
    'locale.en':      'English',
    'locale.ja':      'Japonais',
    'locale.fr':      'Français',
    'locale.zh-Hans': 'Chinois simplifié',
    'locale.pt-BR':   'Portugais (Brésil)',
    'locale.de':      'Allemand',

    'common.loading':    'Chargement…',
    'common.saving':     'Enregistrement…',
    'common.save':       'Enregistrer',
    'common.create':     'Créer',
    'common.creating':   'Création…',
    'common.delete':     'Supprimer',
    'common.edit':       'Modifier',
    'common.cancel':        'Annuler',
    'common.close':         'Fermer',
    'common.remove':        'Supprimer',
    'common.search':        'Rechercher nœuds / actions…',
    'common.history':       'Historique',
    'common.backToList': 'Retour à la liste',
    'common.refresh':    'Actualiser',
    'common.back':       'Retour',
    'common.menu':       'Menu',
    'common.new':        '+ Nouveau',
    'common.add':        '+ Ajouter',
    'common.status':     'Statut',
    'common.actions':    'Actions',
    'common.optional':   '(optionnel)',

    'common.error.unknown': 'Une erreur est survenue.',
    'common.error.fetch':   'Échec du chargement.',
    'common.error.save':    "Échec de l'enregistrement.",
    'common.error.delete':  'Échec de la suppression.',

    'nav.brand':            'NeNe Concierge',
    'nav.scenarios':        'Scénarios',
    'nav.appearance':       'Apparence',
    'nav.credentials':      "Identifiants d'action",
    'nav.settings':         'Paramètres',
    'nav.logout':           'Déconnexion',
    'nav.collapseSidebar':  'Réduire la barre latérale',
    'nav.expandSidebar':    'Développer la barre latérale',

    'auth.appTitle':      'NeNe Concierge',
    'auth.subtitle':      "Connexion à l'administration",
    'auth.emailLabel':    'Adresse e-mail',
    'auth.passwordLabel': 'Mot de passe',
    'auth.signIn':        'Se connecter',
    'auth.signingIn':     'Connexion…',
    'auth.error':         'Échec de la connexion.',

    'scenarios.pageTitle':     'Scénarios',
    'scenarios.empty':         'Aucun scénario.',
    'scenarios.emptyHint':     'Cliquez sur "Nouveau" pour en créer un.',
    'scenarios.loadError':     'Échec du chargement des scénarios.',
    'scenarios.deleteError':   'Échec de la suppression.',
    'scenarios.confirmDeleteTitle': 'Supprimer le scénario',
    'scenarios.confirmDelete': 'Supprimer « {{name}} » ? Cette action est irréversible.',

    'scenario.status.draft':     'Brouillon',
    'scenario.status.published': 'Publié',
    'scenario.status.archived':  'Archivé',

    'scenarioForm.newTitle':        'Nouveau scénario',
    'scenarioForm.metaSaved':       'Métadonnées enregistrées',
    'scenarioForm.graphSaved':      'Graphe enregistré',
    'scenarioForm.nameLabel':       'Nom du scénario',
    'scenarioForm.namePlaceholder': 'ex. : Flux de prise de contact',
    'scenarioForm.descLabel':       'Description (optionnel)',
    'scenarioForm.descPlaceholder': 'Brève description de ce scénario…',
    'scenarioForm.statusLabel':     'Statut',
    'scenarioForm.canvasHint':      "Créez le scénario pour ouvrir l'éditeur de canevas.",
    'scenarioForm.detailsToggle':   'Détails',
    'scenarioForm.saveError':       "Échec de l'enregistrement.",
    'scenarioForm.graphSaveError':  "Échec de l'enregistrement du graphe.",
    'scenarioForm.deleteError':     'Échec de la suppression.',

    'appearance.pageTitle':      'Apparence',
    'appearance.primaryColor':   'Couleur principale',
    'appearance.secondaryColor': 'Couleur secondaire (texte)',
    'appearance.position':       'Position',
    'appearance.trigger':        'Déclencheur',
    'appearance.iconUrl':        "URL de l'icône (optionnel)",
    'appearance.welcomeText':    'Texte de bienvenue (optionnel)',
    'appearance.saved':          '✓ Enregistré.',
    'appearance.saveError':      "Échec de l'enregistrement.",

    'appearance.position.bottomRight': 'En bas à droite',
    'appearance.position.bottomLeft':  'En bas à gauche',
    'appearance.position.topRight':    'En haut à droite',
    'appearance.position.topLeft':     'En haut à gauche',

    'appearance.trigger.pageLoad':   'Au chargement (ouverture automatique)',
    'appearance.trigger.scroll':     'Au défilement',
    'appearance.trigger.exitIntent': 'Intention de sortie',
    'appearance.trigger.manual':     'Manuel (clic sur le bouton uniquement)',

    'credentials.pageTitle':       "Identifiants d'action",
    'credentials.newTitle':        'Nouvel identifiant',
    'credentials.nameLabel':       'Nom',
    'credentials.adapterLabel':    'Adaptateur',
    'credentials.secretHint':      "* Les paramètres sensibles peuvent être mis à jour via l'API après création.",
    'credentials.empty':           'Aucun identifiant.',
    'credentials.saveError':       'Échec de la création.',
    'credentials.confirmDeleteTitle': 'Supprimer l’identifiant',
    'credentials.confirmDelete':   'Supprimer « {{name}} » ? Les scénarios qui le référencent cesseront de fonctionner.',

    'node.type.message':   'Message',
    'node.type.condition': 'Condition',
    'node.type.action':    'Action',
    'node.type.end':       'Fin',

    'node.label':        'Étiquette',
    'node.messageText':  'Texte du message',
    'node.choices':      'Choix rapides',
    'node.addChoice':    '+ Ajouter un choix',
    'node.variableName': 'Nom de variable (pour collecter la saisie)',
    'node.variableHint': 'Nom de variable à référencer dans les nœuds suivants',
    'node.tab.config':       'Config',
    'node.tab.analytics':    'Analytiques',
    'node.tab.connections':  'Connexions',
    'node.edited':           'Modifié',
    'node.conditionVar': 'Variable',
    'node.operator':     'Opérateur',
    'node.compareValue': 'Valeur de comparaison',
    'node.actionType':   "Type d'action",
    'node.credential':   'Identifiant',

    'node.qrCode':               'Code QR',
    'node.qrContent':            'Contenu (URL ou texte)',
    'node.qrContentPlaceholder': 'https://example.com/{{user_id}}',
    'node.qrContentHint':        "L'interpolation {{variable}} est prise en charge.",
    'node.qrVariable':           'Nom de la variable de sortie',
    'node.qrVariablePlaceholder': 'qr_url',
    'node.qrSize':               'Taille de l\'image (px, 64–800)',

    'node.outcome':      'Résultat',
    'node.delete':       '🗑 Supprimer ce nœud',

    'canvas.save':   '💾 Enregistrer',
    'canvas.saving': 'Enregistrement…',

    'canvas.editMode':      'Édition',
    'canvas.analyticsMode': 'Analytique',

    'canvas.analytics.loading':    'Chargement des analytics…',
    'canvas.analytics.noData':     'Aucune donnée de session pour cette période.',
    'canvas.analytics.visits':     '{{count}} visites',
    'canvas.analytics.dropOff':    '{{rate}}% abandon',
    'canvas.analytics.bottleneck': '⚠ Goulot',
    'canvas.analytics.sessions':   'Sessions',
    'canvas.analytics.completed':  'Terminées',
    'canvas.analytics.converted':  'Converties',
    'canvas.analytics.rate':       '{{rate}}%',

    // ── Action Logs ───────────────────────────────────────────────────────────
    'nav.actionLogs':          'Action Logs',
    // ── Theme ─────────────────────────────────────────────────────────────────
    'theme.toggleLight': 'Passer en mode clair',
    'theme.toggleDark':  'Passer en mode sombre',
    // ── Dashboard ────────────────────────────────────────────────────────────
    'nav.dashboard':                  'Tableau de bord',
    'dashboard.pageTitle':            'Tableau de bord',
    'dashboard.loadError':            'Échec du chargement du tableau de bord.',
    'dashboard.sessions7d':           'Sessions (7j)',
    'dashboard.converted7d':          'Conversions (7j)',
    'dashboard.conversionRate':       'Taux de conversion',
    'dashboard.activeSessions':       'Sessions actives',
    'dashboard.publishedScenarios':   'Scénarios publiés',
    'dashboard.actionFailures24h':    "Échecs d'action (24h)",
    'dashboard.dailySessions':        'Sessions quotidiennes (7j)',
    'dashboard.noData':               'Pas encore de données.',
    'dashboard.failuresAlert':        "{{count}} échec(s) d'action dans les 24 dernières heures.",
    // ── Sessions ─────────────────────────────────────────────────────────────
    'nav.sessions':              'Sessions',
    'sessions.pageTitle':        'Sessions',
    'sessions.empty':            'Aucune session pour le moment.',
    'sessions.loadError':        'Échec du chargement des sessions.',
    'sessions.filterOutcome':    'Résultat',
    'sessions.filterConversion': 'Conversion',
    'sessions.all':              'Tous',
    'sessions.active':           'Actif',
    'sessions.completed':        'Terminé',
    'sessions.dropped':          'Abandonné',
    'sessions.converted':        'Converti',
    'sessions.yes':              'Oui',
    'sessions.no':               'Non',
    'sessions.scenarioId':       'Scénario',
    'sessions.startedAt':        'Démarré',
    'sessions.endedAt':          'Terminé à',
    'sessions.outcome':          'Résultat',
    'sessions.conversion':       'Conversion',
    'sessions.detail.title':     'Détail de la session',
    'sessions.detail.variables': 'Variables collectées',
    'sessions.detail.messages':  'Messages',
    'sessions.detail.close':     'Fermer',
    'sessions.detail.loadError': 'Échec du chargement du détail de la session.',
    // ── Settings ──────────────────────────────────────────────────────────────
    'settings.pageTitle':  'Paramètres',
    'settings.adminTheme': "Thème d'administration",

    'theme.label':       'Thème',

    // ══ Locale parity fill (#168) ══════════════════════════════════════════
    // ── Common (locale parity fill) ──
    'common.id':                     'ID',
    'common.name':                   'Nom',
    'common.description':            'Description',
    'common.createdAt':              'Créé le',
    'common.required':               '*',
    // ── Auth placeholders ──
    'auth.emailPlaceholder':         'admin@example.com',
    'auth.pwPlaceholder':            '••••••••',
    // ── Scenario form ──
    'scenarioForm.loadError':        'Échec du chargement du scénario.',
    // ── Appearance ──
    'appearance.iconPlaceholder':    'https://example.com/icon.png',
    'appearance.welcomePlaceholder': 'Comment puis-je vous aider ?',
    'appearance.loadError':          "Échec du chargement de l'apparence.",
    // ── Credentials ──
    'credentials.namePlaceholder':   'ex. : webhook de notification Slack',
    'credentials.loadError':         'Échec du chargement des identifiants.',
    'credentials.deleteError':       'Échec de la suppression.',
    'credentials.adapter.http':      'HTTP (API externe)',
    'credentials.adapter.email':     'E-mail',
    'credentials.adapter.slack':     'Slack',
    'credentials.adapter.chatwork':  'Chatwork',
    // ── Node editor ──
    'node.messagePlaceholder':       'Message à afficher au visiteur…',
    'node.addChoicePrompt':          'Saisir le texte du choix',
    'node.variablePlaceholder':      'ex. : user_name',
    'node.conditionVarPlaceholder':  'ex. : user_answer',
    'node.compareValuePlaceholder':  'Valeur à comparer',
    'node.conditionHint':            'Poignées du bas : gauche = true / droite = false',
    'node.credentialNone':           '— Sélectionner —',
    'node.outcome.completed':        'completed — Terminé',
    'node.outcome.abandoned':        'abandoned — Abandonné',
    'node.operator.eq':              '= Égal',
    'node.operator.neq':             '≠ Différent',
    'node.operator.contains':        'contains contient',
    'node.operator.exists':          'exists existe',
    'node.operator.not_exists':      'not_exists absent',
    'node.addToCanvas':              'Ajouter un nœud {{type}}',
    // ── Action Logs ──
    'actionLogs.pageTitle':          "Journaux d'action",
    'actionLogs.empty':              "Aucun journal d'action pour le moment.",
    'actionLogs.loadError':          "Échec du chargement des journaux d'action.",
    'actionLogs.filterAdapter':      'Adaptateur',
    'actionLogs.filterStatus':       'Statut',
    'actionLogs.all':                'Tous',
    'actionLogs.sessionId':          'Session',
    'actionLogs.scenarioId':         'Scénario',
    'actionLogs.executedAt':         'Exécuté le',
    'actionLogs.error':              'Erreur',
    // ── History (list page) ──
    'nav.history':                   'Historique',
    'history.pageTitle':             'Historique des modifications',
    'history.subtitle':              '{{total}} révisions',
    'history.searchPlaceholder':     'Rechercher par nom ou utilisateur…',
    'history.allOps':                'Toutes les opérations',
    'history.allScenarios':          'Tous les scénarios',
    'history.filter.scenario':       'scénario',
    'history.filter.operation':      'opération',
    'history.filter.from':           'Du',
    'history.filter.to':             'Au',
    'history.col.operation':         'Opération',
    'history.col.scenario':          'Scénario',
    'history.col.user':              'Utilisateur',
    'history.col.nodes':             'Taille',
    'history.col.when':              'Date',
    // ── Pagination ──
    'common.prev':                   'préc.',
    'common.next':                   'suiv.',
    // ── Users ──
    'nav.users':                     'Utilisateurs',
    'users.pageTitle':               'Gestion des utilisateurs',
    'users.subtitle':                '{{total}} utilisateurs · {{active}} actifs',
    'users.empty':                   'Aucun utilisateur pour le moment.',
    'users.loadError':               'Échec du chargement des utilisateurs.',
    'users.saveError':               "Échec de l'enregistrement de l'utilisateur.",
    'users.deleteError':             "Échec de la suppression de l'utilisateur.",
    'users.colEmail':                'E-mail',
    'users.colRole':                 'Rôle',
    'users.colStatus':               'Statut',
    'users.colCreated':              'Créé le',
    'users.new':                     '+ Nouvel utilisateur',
    'users.create.title':            'Nouvel utilisateur',
    'users.edit.title':              "Modifier l'utilisateur",
    'users.field.email':             'E-mail',
    'users.field.password':          'Mot de passe',
    'users.field.passwordHint':      'Au moins 8 caractères. Laissez vide lors de la modification pour conserver le mot de passe actuel.',
    'users.field.role':              'Rôle',
    'users.field.status':            'Statut',
    'users.role.superadmin':         'Super-administrateur',
    'users.role.owner':              'Propriétaire',
    'users.role.editor':             'Éditeur',
    'users.role.viewer':             'Lecteur',
    'users.status.active':           'Actif',
    'users.status.disabled':         'Désactivé',
    'users.confirmDeleteTitle':      "Supprimer l'utilisateur",
    'users.confirmDelete':           'Supprimer « {{email}} » ? Son accès sera révoqué immédiatement.',
    'users.noSelection.title':       'Aucun utilisateur sélectionné',
    'users.noSelection.hint':        'Cliquez sur une ligne pour modifier, ou sur + Nouveau pour créer.',
    // ── Org switcher ──
    'me.currentOrg':                 'Organisation actuelle',
    'me.moreOrgs':                   'de plus',
    'me.noOrg':                      'Aucune organisation',
    // ── History (drawer / timeline) ──
    'history.title':                 'Historique des modifications',
    'history.empty':                 'Aucun historique pour le moment.',
    'history.unknownUser':           'Utilisateur inconnu',
    'history.op.create':             'Créé',
    'history.op.update':             'Modifié',
    'history.op.graph_save':         'Graphe enregistré',
    'history.op.status_change':      'Statut modifié',
    'history.op.delete':             'Supprimé',
    // ── Revision diff ──
    'diff.title':                    'Révision #{{rev}}',
    'diff.loading':                  'Chargement de la révision…',
    'diff.noSnapshot':               'Aucune donnée de capture n’a été enregistrée pour cette révision — différence indisponible.',
    'diff.noChanges':                'Aucune modification structurelle entre cette révision et la précédente.',
    'diff.noPrevious':               '(révision initiale — rien à comparer)',
    'diff.fields':                   'Modifications de champs',
    'diff.nodes':                    'Modifications de nœuds',
    'diff.edges':                    'Modifications de liens',
    'diff.changed':                  'modifié',
    'diff.meta.operation':           'opération',
    'diff.meta.by':                  'par',
    'diff.meta.when':                'date',
    'diff.meta.comparedTo':          'comparé à',
    'diff.noSelection.title':        'Aucune révision sélectionnée',
    'diff.noSelection.hint':         'Cliquez sur une ligne pour voir les modifications.',
}
