...
namespace NeNeConcierge\Tests\Http;
 ...
 *
 * Front-end fleet clients (`@hideyukimori/nene2-client` v1.1.0, adopted by the admin
 * SPA in #164/#165) mirror every bearer token into `X-Authorization: Bearer <token>`
 * so that shared hosting (HETEML-type Tier A) — where an upstream proxy strips the
 * standard `Authorization` header before PHP sees it — can still authenticate.
 * `RuntimeServiceProvider` enables the receiver via
 ...
 * `AdminApiAuthMiddleware` (the bearer auth stage) runs.
 *
 * `GET /api/v1/organizations` is bearer-protected for all methods
 * (`AdminApiAuthMiddleware::ADMIN_ONLY_PREFIXES`) but bypasses org resolution
 * (`OrgResolverMiddleware::BYPASS_PREFIXES`), so these assertions isolate the
 * credential-restoration behaviour with no seeded tenant.
 *
...
final class AuthorizationHeaderFallbackE2ETest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        // Load .env.example as .env when .env does not exist (CI), matching
        // tests/HealthTest.php's convention for booting the runtime container.
     ...
     * here would be downstream authorization — `CapabilityMiddleware` requiring a
     * role claim — which is out of scope for the transport-level mirror proof).
     */
